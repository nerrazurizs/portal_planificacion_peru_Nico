...
def _load_outbound(dims: pd.DataFrame | None = None,
    ...
    """Carga demanda proyectada outbound desde forecast.xlsx (forecast_manual_canal_dist)."""
    if not _FCST_PATH.exists():
        return pd.DataFrame()

    df_raw = pd.read_excel(_FCST_PATH, sheet_name=0, dtype={"id_material": str})
...
def render_inbound(conn):
    ...
    with st.expander("🔍 Debug compras proy (eliminar luego)", expanded=False):
        st.write(f"Parquet path exists: {_PARQUET_PATH.exists()}")

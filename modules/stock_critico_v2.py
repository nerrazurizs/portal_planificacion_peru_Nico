...
from db.queries import (
    QUERY_STOCK_CRITICO_METRICS_V2 as QUERY_STOCK_CRITICO_METRICS,
...
AUTO_METRICS_FILE = "querystockcritico_metrics_v2_auto.csv"
AUTO_DETAIL_FILE = "querystockcritico_detail_auto.csv"   # compartido con V1 (misma query)
...
def _prepare_health_data(df, ventas_px=None, stock_cd_diario=None):
    """Aggregate to SKU level, classify tiers. Returns (df_valid, df_sin_info).

    If ventas_px is provided, uses 6-month adjusted MOI (censored demand)
    instead of the 90-day COGS-based MOI from Snowflake.
    If stock_cd_diario is also provided, uses InStock CD % to filter months.
    """
    if df.empty:
    ...
    # ── MOI Ajustado 6m (si ventas_px disponible) ──
    if ventas_px is not None and not ventas_px.empty:
        sku = calcular_moi_ajustado(sku, ventas_px, stock_cd_diario=stock_cd_diario, min_dias=15)
        # Use MOI_CLASIF (ajustado si ≥15d, bruto 6m si no) as primary MOI
        sku["MOI"] = sku["MOI_CLASIF"]
        sku["ROTACION_UND_MES"] = sku["ROT_6M"]
    else:
        # Fallback: recalculate MOI from 90-day COGS (original logic)
    ...
    # V2: ANTIGUEDAD_MESES == 0 = sin datos FIFO en comex (ej. producto nacional).
    # _classify_health_tier(0) los lleva a "0-3m" — se incluyen en la matriz.
    # Solo excluir si el tier resultante es genuinamente "Sin Info" (NaN/null).
    sku["TIER_ANTIGUEDAD"] = _classify_health_tier(sku["ANTIGUEDAD_MESES"])

    mask_valid = sku["TIER_ANTIGUEDAD"] != "Sin Info"
...
def _download_data(conn):
    ...
    sp = StepProgress(
        ...
        title="Descargando Stock Critico",
    )

    # 1. METRICS
    sp.advance("Descargando Metricas (MOI / Antiguedad)...")
    df_metrics = pd.read_sql(QUERY_STOCK_CRITICO_METRICS, conn)
    df_metrics.columns = [c.upper() for c in df_metrics.columns]
    import csv as _csv
    df_metrics.to_csv(AUTO_METRICS_FILE, index=False, quoting=_csv.QUOTE_ALL)

    # 2. DETAIL (chunked)
    sp.advance("Descargando Detalle por Sucursal (puede tardar)...")
    first_chunk = True
    ...
    # 3. SALES
    sp.advance("Descargando Ventas Historicas...")
    ...
    sp.finish("Datos Actualizados! Generando Dashboard...")
    return None
    ...
    for chunk in pd.read_csv(AUTO_DETAIL_FILE, chunksize=CHUNK_SIZE):
        ...
        for r in chunk_snap.itertuples():
            ...
            if r.SKU_PRODUCTO in crit_skus:
                canal_det = clasificar_canal(r.CANAL_DE_DISTRIBUCION, r.DESCRIPCION_SUCURSAL, cod_bodega=str(getattr(r, "COD_BODEGA", "") or ""))
...
def render_stock_dashboard_v2(conn):
    st.html("<h2 class='sub-header'>Dashboard Stock 2.0</h2>")
    st.info(
        "**Dashboard Stock 2.0** — Stock crítico: **MOI ≥ 12m** (o sin venta) "
        "**AND edad del producto ≥ 12m** (desde 1ª venta en Dorel). "
        "Un producto 'conocido' ya debería tener buen nivel de venta — si tiene MOI alto, es un problema real. "
        "La **antigüedad del stock físico** (FIFO comex) se muestra como contexto adicional en el detalle."
    )
    ...
    try:
        ...
        df_m = pd.read_csv(AUTO_METRICS_FILE, on_bad_lines="warn")
        ...
        # V2: ANTIGUEDAD_MESES = edad del producto desde 1ª venta VCM (MESES_EN_CIA)
        # Criterio crítico: producto "conocido" (lleva en Dorel >= 12m) con MOI alto.
        # ANTIGUEDAD_STOCK_MESES (FIFO) se conserva como columna de contexto adicional.
        for _c in ["ANTIGUEDAD_STOCK_MESES", "MESES_EN_CIA", "MOI", "STOCK_COSTO", "STOCK_UNIDADES"]:
            if _c in df_m.columns:
                df_m[_c] = pd.to_numeric(df_m[_c], errors="coerce").fillna(0)
        if "MESES_EN_CIA" in df_m.columns:
            df_m["ANTIGUEDAD_MESES"] = df_m["MESES_EN_CIA"]
        elif "ANTIGUEDAD_STOCK_MESES" in df_m.columns:
            # Fallback si no hay datos VCM
            df_m["ANTIGUEDAD_MESES"] = df_m["ANTIGUEDAD_STOCK_MESES"]

        df_m = apply_pm_filter(df_m)
        latest_date_metrics = df_m["FECHA"].max()

        # Load ventas_px and stock_cd_diario for adjusted MOI calculation
        try:
            from utils.filters import norm_cols as _nc
            _ventas_px = _nc(cq.ventas_mensual_precio(conn))
        except Exception:
            _ventas_px = pd.DataFrame()
        try:
            _stock_cd_d = _nc(cq.stock_cd_diario_6m(conn))
        except Exception:
            _stock_cd_d = pd.DataFrame()

        # Identify critical SKUs
        df_last_m = df_m[df_m["FECHA"] == latest_date_metrics].copy()

        # ── Recalcular MOI con demanda censurada 6m + InStock CD ──
        if not _ventas_px.empty and "SKU_PRODUCTO" in df_last_m.columns:
            _sku_agg = df_last_m.groupby("SKU_PRODUCTO", as_index=False)["STOCK_COSTO"].sum()
            _sku_agg = calcular_moi_ajustado(
                _sku_agg, _ventas_px,
                stock_cd_diario=_stock_cd_d if not _stock_cd_d.empty else None,
                min_dias=15,
            )
            # Map MOI_CLASIF back to df_last_m
            _moi_map = _sku_agg.set_index("SKU_PRODUCTO")["MOI_CLASIF"]
            _rot_map = _sku_agg.set_index("SKU_PRODUCTO")["ROT_6M"]
            df_last_m["MOI"] = df_last_m["SKU_PRODUCTO"].map(_moi_map).fillna(df_last_m["MOI"])
            if "ROTACION_UND_MES" not in df_last_m.columns:
                df_last_m["ROTACION_UND_MES"] = df_last_m["SKU_PRODUCTO"].map(_rot_map).fillna(0)

        cond_moi_crit = (df_last_m["MOI"] >= 12) | (df_last_m["MOI"].isna())
        _age_col_crit = df_last_m["MESES_EN_CIA"] if "MESES_EN_CIA" in df_last_m.columns else df_last_m.get("ANTIGUEDAD_MESES", 0)
        cond_ant_crit = (_age_col_crit >= 12) & (_age_col_crit.notna() if hasattr(_age_col_crit, 'notna') else True)
        ...
        _pm_active = st.session_state.get("sidebar_pm_filter", "Todos") != "Todos"
        _g_any_filter = _pm_active or any([
        ...
        _age_col_all = df_m["MESES_EN_CIA"] if "MESES_EN_CIA" in df_m.columns else df_m.get("ANTIGUEDAD_MESES", 0)
        cond_ant_all = (_age_col_all >= 12) & (_age_col_all.notna() if hasattr(_age_col_all, 'notna') else True)
        ...
        top_risk = (
            df_last_m[((df_last_m["MOI"] >= 12) | (df_last_m["MOI"].isna())) & ((df_last_m["MESES_EN_CIA"] if "MESES_EN_CIA" in df_last_m.columns else df_last_m.get("ANTIGUEDAD_MESES", 0)) >= 12)]
        ...
        if "LINEA" in df_last_m.columns:
            _age_col_fin = df_last_m["MESES_EN_CIA"] if "MESES_EN_CIA" in df_last_m.columns else df_last_m.get("ANTIGUEDAD_MESES", 0)
            _crit_mask_fin = (
                ((df_last_m["MOI"] >= 12) | (df_last_m["MOI"].isna()))
                & (_age_col_fin >= 12)
                & (_age_col_fin.notna() if hasattr(_age_col_fin, 'notna') else True)
            )
            _crit_lines = (
        ...
        if _df_9b.empty:
            ...
            _df_valid, _df_sin_info = _prepare_health_data(_df_9b, ventas_px=_ventas_px, stock_cd_diario=_stock_cd_d)
            ...
            if _sin_info_n > 0:
                st.caption(f"{_sin_info_n} SKUs excluidos por antigüedad no clasificable (valor fuera de rangos esperados).")
            st.html("<br>")
            ...
            if not _filtered.empty:
                _show_cols = ["SKU_PRODUCTO", "NOM_PRODUCTO", "AREA", "LINEA", "MARCA",
                              ...
                              "ANTIGUEDAD_MESES", "ANTIGUEDAD_STOCK_MESES"]
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "ANTIGUEDAD_MESES": st.column_config.NumberColumn(
                            "Edad Producto (m)", format="%.0f",
                            help="Meses desde la primera venta en Dorel (VCM) — base del criterio crítico"
                        ),
                        "ANTIGUEDAD_STOCK_MESES": st.column_config.NumberColumn(
                            "Ant. Stock FIFO (m)", format="%.1f",
                            help="Meses desde que llegó la unidad más antigua aún en bodega (FIFO comex)"
                        ),
                        "MESES_EN_CIA": st.column_config.NumberColumn(
                            "Edad Producto (m)", format="%.0f",
                            help="Meses desde la primera venta real en VCM"
                        ),
                    },
            ...
            if not _df_sin_info.empty:
                ...
                with st.expander(f"SKUs sin clasificar ({_si_n} SKUs · {_fmt_cl(_si_stock)})"):
                    ...
                    _si_cols = ["SKU_PRODUCTO", "NOM_PRODUCTO", "AREA", "LINEA", "STOCK_COSTO", "MOI", "ANTIGUEDAD_MESES", "ANTIGUEDAD_STOCK_MESES"]
        ...
        if not df_sales_crit.empty and "UNIDADES_VENDIDAS" in df_sales_crit.columns:
            ...
            _scatter_cols = ["SKU_PRODUCTO", "STOCK_COSTO", "MOI", "ANTIGUEDAD_MESES", "ANTIGUEDAD_STOCK_MESES"]
        ...
        if st.button("Generar Informe Excel con Unidades"):
            with lottie_spinner("export"):
                try:
                    ...
                    _age_col_exp = df_last_m["MESES_EN_CIA"] if "MESES_EN_CIA" in df_last_m.columns else df_last_m.get("ANTIGUEDAD_MESES", 0)
                    cond_ant = _age_col_exp >= 12

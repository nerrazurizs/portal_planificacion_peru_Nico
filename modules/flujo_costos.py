...
def _compute_transitos(df_pos: pd.DataFrame, factor_ovr: dict) -> pd.DataFrame:
    ...
    # Precio unitario y cantidad pendiente para calcular monto pendiente en moneda orig.
    precio_unit = pd.to_numeric(df.get("PRECIO_UNITARIO", pd.Series(0, index=df.index)), errors="coerce").fillna(0)
    proc        = df.get("PROCEDENCIA", pd.Series("", index=df.index)).fillna("").astype(str).str.upper().str.strip()
    mask_imp    = proc == "IMPORTADO"

    # Factor base parquet (0 → 1.3)
    ...
    base_amount = precio_unit * df["QTY_PENDIENTE"]
    df["AMOUNT_SOLES"] = np.where(mask_imp, base_amount * factor * tc, base_amount)
    ...
    # Solo ETA año en curso (2026); NaT se mantiene para reasignar luego
...
def _render_diagnostico_compras(factor_ovr: dict, df_pos_raw: pd.DataFrame):
    ...
    with st.expander("🔍 Diagnóstico Compras Pry S/. y TT (detalle por línea)", expanded=False):
        ...
        areas_disp = sorted(df_yr["AREA"].unique())
        col_a, col_l = st.columns(2)
        sel_area  = col_a.selectbox("Área",  options=areas_disp,  key="diag_area")
        lineas_disp = sorted(df_yr[df_yr["AREA"] == sel_area]["LINEA"].unique())
        sel_linea = col_l.selectbox("Línea", options=lineas_disp, key="diag_linea")

        df_sel = df_yr[
            (df_yr["AREA"]  == sel_area) &
            (df_yr["LINEA"] == sel_linea) &
            (df_yr["FORECAST_COMPRA"] > 0)
        ].copy()
        ...
        if df_sel.empty:
            ...
            st.metric(f"Total año {hoy.year} — {sel_area} / {sel_linea}", f"S/ {total_cp:,.0f}")
        ...
        df_tt_sel = df_tt[
            (df_tt["AREA"]  == sel_area) &
            (df_tt["LINEA"] == sel_linea)
        ].copy()

        if df_tt_sel.empty:
            st.info(f"Sin tránsitos activos para {sel_area} / {sel_linea}.")
            return
        ...
        st.metric(f"Total TT S/. — {sel_area} / {sel_linea}", f"S/ {total_tt:,.0f}")
...
def render_flujo_costos(conn):
    ...
    with lottie_spinner("snowflake"):
        try:
            ...
            df_pos    = _fetch_pos(_cid,    _conn=conn)
        except Exception as e:
